...
/* -------------------------------------------------------------------------- */
//
// A second, weaker allowlist, for the internal documents at /contribute.
//
// It exists because the two questions are genuinely different. "May this
// person read the contributor brief" and "may this person see every customer's
// email address, the revenue figures and the moderation queue" had one answer
// between them while ADMIN_EMAILS was the only gate, which meant onboarding
// somebody to the codebase required handing them the console as well.
//
// Admins pass automatically. Nobody should have to be added twice, and an
// admin who cannot read the docs would be a confusing thing to explain.

export function contributorEmails(env: Env["Bindings"]): Set<string> {
  return new Set(
    (env.CONTRIBUTOR_EMAILS ?? "")
      .split(",")
      .map((email) => email.trim().toLowerCase())
      .filter(Boolean),
  );
...
export function isContributorEmail(env: Env["Bindings"], email: string | null | undefined): boolean {
  if (!email) return false;
  if (isAdminEmail(env, email)) return true;
  return contributorEmails(env).has(email.toLowerCase());
}

/**
 * The gate on /contribute.
 *
 * A 404 rather than a 403, for the same reason the console uses one: a stranger
 * should not learn the route is real. Unset CONTRIBUTOR_EMAILS with no admins
 * configured either switches the whole thing off, which is what a fresh clone
 * of this repo should do.
 */
export async function requireContributor(c: Context<Env>, next: Next): Promise<Response | void> {
  const user = c.get("user");
  if (!isContributorEmail(c.env, user?.email)) return c.notFound();
